	...
	function CreateColorPicker() {
		...
		var colorBlock = $("<div>", {
			...
			.mousedown(function(event) {
				...
				function UpdatePosition(left, top) {
					var calcLeft,
						calcTop,
						minusLeft = left - pos.left,
						minusTop = top - pos.top;
					if(minusLeft < 0) {
						calcLeft = 0
					} else if(minusLeft > 150) {
						calcLeft = 150
					} else {
						calcLeft = minusLeft;
					};
		...
		colorBar.mousedown(function(event) {
			...
			var pos = colorBar.offset(),
				maxTop = pos.top + colorBar.height() - 5;
			...
			function UpdatePosition(top) {
				...
				colorBarDragger.css({
					top: top - pos.top + 8 // Add 9 for parent panels' padding.
				});
		...
		colorPicker.setColor = function(color) {
			...
			$('header').css('background', textColor);
			$('#range>div').css('background', textColor);
	...
	*/
	$.fn.colorPicker = function(options) {
		...
		return this.each(function() {
			...
			$('header').css('background', rgbaColor);
			$('#range>div').css('background', rgbaColor);
		});
	};
})(jQuery);